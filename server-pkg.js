...
function getAssetPath(filename) {
  // pkg bundles assets relative to the snapshot
  if (process.pkg) {
...
server.listen(PORT, async () => {
  ...
  try {
    ...
    // Use --no-tls-verify since we have a self-signed cert locally
...
function broadcastBreakoutsState(roomId) {
  ...
  meeting.participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify(payload)));
}
...
function generateInviteToken(clientId) {
  ...
  if (!global.tunnelUrl && !global.tunnelFailed) {
    // Tunnel still starting, wait and retry
...
function joinRoom(clientId, roomId, passphrase, token) {
  ...
  participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify({ type: 'peer-joined', peerId: clientId })));
  broadcastBreakoutsState(roomId);
}
...
function leaveRoom(clientId, roomId) {
  ...
  meeting.subRooms?.get(subRoomId)?.participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify({ type: 'peer-left', peerId: clientId })));
  if (meeting.hostId === clientId && meeting.participants.size > 0) {
    ...
    meeting.participants.forEach(pid => { if (pid !== newHostId) clients.get(pid)?.ws.send(JSON.stringify({ type: 'new-host', hostId: newHostId })); });
...
function moveClientToSubRoom(clientId, roomId, nextSubRoomId) {
  ...
  prevRoom?.participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify({ type: 'peer-left', peerId: clientId })));
  nextRoom.participants.forEach(pid => { if (pid !== clientId) clients.get(pid)?.ws.send(JSON.stringify({ type: 'peer-joined', peerId: clientId })); });
...
function relaySignal(clientId, msg) {
  ...
  clients.get(msg.targetId)?.ws.send(JSON.stringify({ type: 'signal', fromId: clientId, signal: msg.signal }));
}

function relayKeyExchange(clientId, msg) {
  if (!canRelayToTarget(clientId, msg.targetId)) return;
  clients.get(msg.targetId)?.ws.send(JSON.stringify({ type: 'key-exchange', fromId: clientId, publicKey: msg.publicKey }));
}

function relayEncryptedSignal(clientId, msg) {
  if (!canRelayToTarget(clientId, msg.targetId)) return;
  clients.get(msg.targetId)?.ws.send(JSON.stringify({ type: 'encrypted-signal', fromId: clientId, encrypted: msg.encrypted }));
}
...
function kickParticipant(clientId, targetId) {
  ...
  if (!client?.isHost) return;
  const target = clients.get(targetId);
  if (target) { target.ws.send(JSON.stringify({ type: 'kicked', message: 'You have been removed from the meeting' })); leaveRoom(targetId, client.roomId); target.ws.close(); }
}

function muteParticipant(clientId, targetId) {
  const client = clients.get(clientId);
  if (!client?.isHost) return;
  clients.get(targetId)?.ws.send(JSON.stringify({ type: 'mute-request', message: 'Host has requested you to mute' }));
}
...
function lockRoom(clientId) {
  ...
  if (!client?.isHost) return;
  const room = rooms.get(client.roomId);
  if (room) { room.locked = true; room.participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify({ type: 'room-locked' }))); }
}
...
function unlockRoom(clientId) {
  ...
  if (!client?.isHost) return;
  const room = rooms.get(client.roomId);
  if (room) { room.locked = false; room.participants.forEach(pid => clients.get(pid)?.ws.send(JSON.stringify({ type: 'room-unlocked' }))); }
}
...
function rateLimitCheck(clientId) {
  ...
  if (!data) return true;
  if (data.blocked) {
    if (Date.now() - data.blockedAt >= getBlockDuration(data.blockCount)) { data.blocked = false; data.blockedAt = null; data.attempts = []; return true; }
    return false;
  }
