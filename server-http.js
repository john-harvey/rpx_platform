...
console.log('⚠️  Running in HTTP mode (no SSL) - for local testing only');
console.log('⚠️  Certificate pinning disabled in HTTP mode');
...
const server = createServer((req, res) => {
  ...
  if (req.url === '/') {
    ...
    res.writeHead(200, { 'Content-Type': 'application/javascript' });
    res.end(readFileSync(join(__dirname, 'crypto.js')));
...
function handleMessage(clientId, msg) {
  switch (msg.type) {
    case 'verify-server-identity':
      // Skip verification in HTTP mode
      const client = clients.get(clientId);
      if (client) {
        client.ws.send(JSON.stringify({
          type: 'server-identity-verified',
          signature: 'http-mode-no-verification'
        }));
      }
      break;
    case 'create-room':
...
server.listen(PORT, () => {
  ...
  console.log('⚠️  HTTP mode - no encryption at transport layer');
  console.log('⚠️  Certificate pinning disabled');
  console.log('⚠️  For production, use HTTPS (npm start)\n');
});
